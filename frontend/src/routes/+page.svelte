...
<div class="d-flex flex-nowrap h-100" id="app" data-start-date={ data.ctfConfig.start_date } data-tick-length={ data.ctfConfig.tick_length } data-refresh-rate={ data.ctfConfig.refresh_rate }>
    <nav class="d-flex flex-column align-items-stretch flex-shrink-0 bg-body shadow nav-flow">
        <div class="w-100">
            <button id="autoUpdateBtn" title="Refresh flow list" class="btn rounded-0 w-100">
                Auto-Update: ON
            </button>
        </div>
        <div class="d-flex flex-nowrap border-bottom">
            <select class="form-select form-select-sm rounded-0 border-0" id="services-select">
                ...
                {#each data.ctfConfig.services as service}
                    <optgroup label={service.name} data-ipports={ service.ipaddr_ports.join(" ") }>
                        {#if service.ipaddr_ports.length > 1}
                            <option value={ service.ipaddr_ports.join(", ") }>All ({service.name})</option>
                        {/if}
                        {#each service.ipaddr_ports as ipaddr_port}
                            <option value={ipaddr_port}>{ipaddr_port} ({service.name})</option>
                        {/each}
                    </optgroup>
                {/each}
            </select>
            <button class="btn btn-sm border-start rounded-0" data-bs-toggle="modal" data-bs-target="#servicesModal" title="Customize services" aria-label="Service settings">
                <i class="bi bi-gear-fill"></i>
            </button>
            <div class="dropend position-static border-start" id="dropdown-filter">
                <button class="btn text-nowrap rounded-0" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Dropdown filter">
...
<div id="tick-progress-container" class="position-fixed bottom-0 start-0 end-0 bg-dark bg-opacity-90 border-top border-secondary" style="height: 40px; z-index: 1000;">
    <div class="container-fluid h-100">
        <div class="row align-items-center h-100 px-2">
            <div class="col-auto">
                <small class="text-light fw-bold" id="tick-info">Tick 0</small>
            </div>
            <div class="col">
                <div class="progress" style="height: 8px;">
                    <div id="tick-progress-bar"
            ...
            <div class="col-auto">
                <small class="text-light" id="tick-timer">00:00</small>
            </div>
        </div>
    </div>
</div>